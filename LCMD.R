...
rs_wmse <- function(actual, pred_new, 
 ...
 # Returns a vector with the weighted mean squared error from 
 ...
 for (i in 1:n_iter){
  ...
  error_sq <- (temp$pred_new - temp$actual)^2 
  errors[i] <- weighted.mean(error_sq, temp$weight)
 }
 ...
 if (plot == TRUE) {
  windows()
  boxplot(errors, 
    main = "Random Sample Distribution of WMSE",
    ylab = "WMSE")
 }
...
rs_wmean_n_power_error <- function(actual, pred_new, n_power,
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE) {
 # Returns the weighted mean n-power error from random samples of 
 # the data set as a vector.
 # 
 # Inputs:
 #  actual: a vector of actual values
 #  pred_new: a vector of predicted values
 ...
 for (i in 1:n_iter){
  ...
  ## calculate the n-power error
  error_sq = (temp$pred_new - temp$actual)^ n_power
  errors[i] <- weighted.mean(error_sq, temp$weight)
 }
 ...
 if (plot == TRUE){
  ...
  plot_sub <- paste('Weigthed Mean', n_power, 'Power Error')
  y_text <- paste('Weigthed Mean', n_power, 'Power Error')
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
    ylab = y_text)
...
## Pseudo R^2
rs_pseudo_r_sq <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250,  policy_id=NULL, rbl = TRUE, plot = TRUE) {
 # Returns a vector with the pseudo R^2 from random samples of 
 # the data set.
 # 
 # Inputs:
 #  actual: a vector of actual values
 ...
 for (i in 1:n_iter){
  ...
  intercept <- weighted.mean(temp$actual, temp$weight)
  error_intercept_sq <- (temp$actual - intercept)^2
  wmse_intercept <- weighted.mean(error_intercept_sq, temp$weight)
  
  ## Calc WMSE Model
  error_sq = (temp$pred_new - temp$actual)^2
  wmse <- weighted.mean(error_sq, temp$weight)
  
  ## Return the pseudo_r_sq
  errors[i] <- 1 - wmse / wmse_intercept
 }
 if (plot == TRUE){
  windows()
  plot_main <- 'Random Sample Distribution'
  plot_sub <- 'Pseudo R-square'
  y_text <- 'Pseudo R-square'
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
    ylab = y_text)
...
## Cov (y, y_hat)
rs_model_cov <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE){
 # Returns a vector of the weighted covariance between the actual vector
 # and the pred_new vector from random samples of the data set.
 # 
 # Inputs:
 #  actual: a vector of actual values
 #  pred_new: a vector of predicted values
 ...
 for (i in 1:n_iter){
  ...
  d <- data.frame(model = temp$pred_new, actual = temp$actual)
  c <- cov.wt(d, wt = temp$weight)
  ## cov.wt return a n x n covariance matrix
  ## diagonal entries are variances
  ## off-diagonal entries are covariance
  errors[i] <- c$cov[1,2]
 ...
 if (plot == TRUE){
  ...
  plot_main <- 'Random Sample Distribution'
  plot_sub <- 'Covariance(actual, predicted)'
  y_text <- 'Covariance'
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
...
## Cor (y, y_hat)
rs_model_cor <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE){
 # Returns a vector of the weighted correlation between the actual vector
 # and the pred_new vector from random samples of the data set.
 # 
 # Inputs:
 #  actual: a vector of actual values
 ...
 for (i in 1:n_iter){
  ...
  ## Calculate covariances, variances and correlation
  d <- data.frame(model = temp$pred_new, actual = temp$actual)
  c <- cov.wt(d, wt = temp$weight)
  ## cov.wt return a n x n covariance matrix
  ## diagonal entries are variances
  ## off-diagonal entries are covariance
  errors[i] <- c$cov[1,2]/ (sqrt(c$cov[1,1])* sqrt(c$cov[2,2]))
 }
 if (plot == TRUE){
  windows()
  
  plot_main <- 'Random Sample Distribution'
  plot_sub <- 'Correlation(actual, predicted)'
  y_text <- 'Correlation'
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
...
## Chi Squared
rs_chi_square <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE){
 # Returns a vector of the weighted chi-square from random samples of 
 # the data set.
 # 
 # Inputs:
 #  actual: a vector of actual values
 ...
 for (i in 1:n_iter){
  ...
  ## Find the prediction values that are not zeros
  ind <- temp$pred_new != 0

  ## Calculate the chi-square, filtering out the zero predictions
  d <- data.frame(model = temp$pred_new[ind], actual = temp$actual[ind], 
    weight = weight[ind])
  e <- (d$actual - d$model)^2 / d$model
  errors[i] <- sum(d$weight * e)/sum(d$weight)
 }
 if (plot == TRUE){
  windows()
...
## percentage error
rs_percentage_error <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE){
 # Returns a vector of the absolute value percentage error
 # from random samples of the data set.
 #
 # Inputs:
 #  actual: a vector of actual values
 #  pred_new: a vector of predicted values
 ...
 for (i in 1:n_iter){
  ...
  ## Calculate the absolute precentage errors
  d <- data.frame(model = temp$pred_new, actual = temp$actual, weight = weight)
  index <- d$model > 0
  e <- abs(d$actual[index]/d$model[index] - 1)
  errors[i] <-weighted.mean(e, d$weight[index])
 }
 if (plot == TRUE){
  windows()
  
  plot_main <- 'Random Sample Distribution'
  plot_sub <- 'Percentage Error'
  y_text <- 'Percentage Error'
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
    ylab = y_text)
...
rs_wmad <- function(actual, pred_new, 
  weight = rep(1, length(actual)), n_sample = length(actual), 
  n_iter = 250, policy_id=NULL, rbl = TRUE, plot = TRUE){
 # Returns the weighted mean absolute deviation from random samples of 
 # the data set as a vector.
 # 
 # Inputs:
 #  actual: a vector of actual values
 ...
 for (i in 1:n_iter){
  ...
  d <- data.frame(model = temp$pred_new, actual = temp$actual, weight = weight)
  e <- abs(d$actual - d$model)
  errors[i] <- weighted.mean(e, d$weight)
 }
 if (plot == TRUE){
  windows()
  
  plot_main <- 'Random Sample Distribution'
  plot_sub <- 'Weighted Mean Absolute Deviation'
  y_text <- 'Weighted Mean Absolute Deviation'
  boxplot(errors, 
    main = plot_main,
    sub = plot_sub,
    ylab = y_text)
